...
+ The medication items in drone can only be loaded if it is not in the below state. 
...
+ Once medication items loaded into Drone. The state of both medication and drone changes in every 5 second.
...
+ Drone battery will be charged automatically if drone state is IDLE and battery is less than 100 percent. 
+ Drone battery will be increased by 10% for every 10 second.
